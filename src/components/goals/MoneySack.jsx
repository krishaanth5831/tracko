...
// progress: 0..1 — fills the sack bottom-up via a clipped rect
export function MoneySack({ progress, color = '#f59e0b' }) {
  ...
  return (
    <svg viewBox="0 0 120 122" className="w-36 h-36 mx-auto drop-shadow-lg">
      <defs>
      ...
      <path d={SACK_PATH} fill="rgba(255,255,255,0.06)" />

      <g clipPath="url(#sackClip)">
        <rect className="sack-fill" x="0" y={fillY} width="120" height={fillH + 4} fill="url(#fillGrad)" />
        {p > 0.02 && (
          <ellipse className="sack-fill" cx="60" cy={fillY} rx="42" ry="4" fill={color} opacity="0.9" />
        )}
      </g>

      <path d={SACK_PATH} fill="none" stroke="rgba(255,255,255,0.35)" strokeWidth="2.5" />
